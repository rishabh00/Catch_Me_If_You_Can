...
void RGBmatrixPanel::init(uint8_t rows, bool dbuf, uint16_t width) {
...
RGBmatrixPanel::RGBmatrixPanel(bool dbuf, uint16_t width) :
...
void RGBmatrixPanel::updateDisplay(void) {
  ...
  P_OE.setHigh();         // Disable LED output during row/plane switchover
  P_LATCH.setHigh();      // Latch data loaded during *prior* interrupt
  P_CLOCK.setLow();      // Start the clock LOW
  ...
  if(++plane >= nPlanes) {      // Advance plane counter.  Maxed out?
    ...
    (row & 0x1) ? P_addrA.setHigh() : P_addrA.setLow();
    (row & 0x2) ? P_addrB.setHigh() : P_addrB.setLow();
    (row & 0x4) ? P_addrC.setHigh() : P_addrC.setLow();
    if(nRows > 8) {
      (row & 0x8) ? P_addrD.setHigh() : P_addrD.setLow();
    }
  }
  ...
  if(plane > 0) {
    ...
    for(i=0; i < WIDTH; i++) {
        ...
        pinResetFast(_sclk);
#else
        (bits & 0x04) ? P_R1.setHigh() : P_R1.setLow();      //R1
        (bits & 0x08) ? P_G1.setHigh() : P_G1.setLow();       //G1
        (bits & 0x10) ? P_B1.setHigh() : P_B1.setLow();       //B1
        (bits & 0x20) ? P_R2.setHigh() : P_R2.setLow();       //R2
        (bits & 0x40) ? P_G2.setHigh() : P_G2.setLow();       //G2
        (bits & 0x80) ? P_B2.setHigh() : P_B2.setLow();       //B2
        P_CLOCK.setHigh();      //hi
        P_CLOCK.setLow();    //lo
#endif
    }
